...
public:
    ...
    void setData(callback_data *data) {
      cb_data = data;
    }

    oboe::DataCallbackResult
    onAudioReady(oboe::AudioStream *stream, void *audioData, int32_t num_frames) {
        ...
        LOGD(
                ...
                (stream->getDirection() == oboe::Direction::Input) ? "record" : "playout", num_frames,
...
void log_current_settings(oboe::AudioStream *playout_stream,
    ...
    LOGD("playout current_buffer_size: %d", playout_stream->getBufferCapacityInFrames());
    ...
    LOGD("record current_buffer_size: %d", record_stream->getBufferSizeInFrames());
    LOGD("record buffer_capacity: %d", record_stream->getBufferCapacityInFrames());
...
Java_com_facebook_audiolat_MainActivity_runOboe(JNIEnv *env,
                                                   jobject /* this */,
                                                   jobject settings) {
...
//builder.setDeviceId(AAUDIO_UNSPECIFIED);  // more of this later...device_id);
  ...
  LOGD("Open play stream: %d, audio api: %d", result, play_stream->getAudioApi());
