...
#include <dlfcn.h>
#define LOGD(...)  __android_log_print(ANDROID_LOG_DEBUG, "latencycheck", __VA_ARGS__)
...
aaudio_data_callback_result_t dataCallback(
        ...
        aaudio_stream_state_t play_state = AAudioStream_getState(cb_data->play_stream);
        ...
        if (stream == cb_data->rec_stream) {
            ...
            if (time_sec - last_ts > 2){
                ...
                fwrite((int16_t*)audioData, sizeof(int16_t),(size_t)num_frames - cb_data->start_signal_size, cb_data->file);
                fwrite(cb_data->start_signal, (size_t)cb_data->start_signal_size, sizeof(int16_t), cb_data->file);
                buffer_index = 0;
                last_ts = time_sec;
            } else {
            ...
            if (playing && play_state == AAUDIO_STREAM_STATE_STARTED &&
                ...
                memcpy(audioData, cb_data->pattern + buffer_index, sizeof(int16_t) * num_frames);
...
JNICALL
Java_com_facebook_latencycheck_MainActivity_runAAudio(
    JNIEnv *env,
            jobject /* this */,
            jobject settings) {

    jclass cSettings = env->GetObjectClass(settings);
    jfieldID fid = env->GetFieldID(cSettings, "reference", "Ljava/nio/ByteBuffer;");
    ...
    AAudioStreamBuilder_setDeviceId(play_builder, AAUDIO_UNSPECIFIED);// more of this later...device_id);
    AAudioStreamBuilder_setDirection(play_builder, AAUDIO_DIRECTION_OUTPUT);
    AAudioStreamBuilder_setSharingMode(play_builder, AAUDIO_SHARING_MODE_SHARED); //AAUDIO_SHARING_MODE_EXCLUSIVE no available
    ...
    AAudioStreamBuilder_setPerformanceMode(play_builder, AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
    ...
    AAudioStreamBuilder_setSharingMode(rec_builder, AAUDIO_SHARING_MODE_SHARED); //AAUDIO_SHARING_MODE_EXCLUSIVE no available
    ...
    AAudioStreamBuilder_setPerformanceMode(rec_builder, AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
    AAudioStreamBuilder_setBufferCapacityInFrames(rec_builder, 64);
    AAudioStreamBuilder_setInputPreset(rec_builder, AAUDIO_INPUT_PRESET_UNPROCESSED);
    ...
    // print current settings
    {
        ...
        int play_buffer_capacity = AAudioStream_getBufferCapacityInFrames(play_stream);
        int play_current_buffer_size = AAudioStream_getBufferSizeInFrames(play_stream);

        int rec_frame_per_burst = AAudioStream_getFramesPerBurst(rec_stream);
        int rec_buffer_capacity = AAudioStream_getBufferCapacityInFrames(rec_stream);
        int rec_current_buffer_size = AAudioStream_getBufferSizeInFrames(rec_stream);
        ...
        LOGD("Play Performance mode: %d", AAudioStream_getPerformanceMode(play_stream));
        ...
        LOGD("Rec Performance mode: %d", AAudioStream_getPerformanceMode(rec_stream));
    }
...
cleanup:
    LOGD("Cleanup");
    if (play_stream)
        AAudioStream_close(play_stream);
    if (rec_stream)
        AAudioStream_close(rec_stream);
    if (play_builder)
        AAudioStreamBuilder_delete(play_builder);
    if (rec_builder)
        AAudioStreamBuilder_delete(rec_builder);
    return 0;
}
